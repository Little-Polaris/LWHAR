...
class st_attention(nn.Module):
    def __init__(self, num_point, in_channels, out_channels, num_head=3, residual=True):
        super(st_attention, self).__init__()
        self.num_head = num_head
        self.pe = pos_encoding(in_channels, num_point)
        self.attentions = nn.ModuleList()
        for i in range(self.num_head):
            self.attentions.append(st_attention_block(in_channels, out_channels))
    ...
    def forward(self, x):
        ...
        for i in range(self.num_head):
            x1 += self.attentions[i](x, self.A[i])
...
class sts_feature_extraction(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, ):
        ...
        # self.block4 = st_feature_extraction_block(in_channels, out_channels // 4, 1, stride, 1, 'avg')
...
class sts_attention(nn.Module):
    def __init__(self,num_point, in_channels, out_channels, stride=1, residual=True):
        ...
        self.gcn1 = st_attention(num_point, in_channels, out_channels)
        self.tcn1 = sts_feature_extraction(out_channels, out_channels, stride=stride)
        self.relu = nn.ReLU(inplace=True)
        if not residual:
