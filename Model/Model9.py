...
class st_attention_block(nn.Module):
    def __init__(self, in_channels, out_channels, rel_reduction=8, mid_reduction=1):
        ...
        self.W_K = nn.Conv2d(self.in_channels, self.rel_channels//2, kernel_size=1)
        self.W_Q = nn.Conv2d(self.in_channels, self.rel_channels//2, kernel_size=1)
        self.W_V = nn.Conv2d(self.in_channels, self.out_channels, kernel_size=1)
        self.permutation = Permutation(25)
        self.attention = attention(self.rel_channels//2, self.rel_channels//2)
    ...
    def forward(self, x, A=None, alpha=1):
        ...
        x = x * alpha + A.unsqueeze(0).unsqueeze(0)
...
class sts_attention(nn.Module):
    def __init__(self, in_channels, out_channels, A, stride=1, residual=True, adaptive=True, kernel_size=5, dilations=[1,2]):
        ...
        self.gcn1 = st_attention(in_channels, out_channels, A, adaptive=adaptive)
        self.tcn1 = sts_feature_extraction(out_channels, out_channels, kernel_size=kernel_size, stride=stride, dilations=dilations,
                                           residual=False)
        self.relu = nn.ReLU(inplace=True)
        if not residual:
            self.residual = lambda x: 0
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        base_channel = 64
        self.l1 = sts_attention(in_channels, base_channel, A, residual=False, adaptive=adaptive)
        self.l2 = sts_attention(base_channel, base_channel, A, adaptive=adaptive)
        self.l3 = sts_attention(base_channel, base_channel * 2, A, stride=2, adaptive=adaptive)
        self.l4 = sts_attention(base_channel * 2, base_channel * 2, A, adaptive=adaptive)
        self.l5 = sts_attention(base_channel * 2, base_channel * 4, A, stride=2, adaptive=adaptive)
        self.l6 = sts_attention(base_channel * 4, base_channel * 4, A, adaptive=adaptive)
